...
- **`ads-monitor/`** (monitor de campanhas Product Ads/Mercado Ads, 3 camadas determinísticas + 1 opcional): coleta (`coletar.py`) grava snapshot diário de campanha **e de ad_group/item dentro dela** (atribuição por SKU, best-effort) num SQLite local, **com agendamento diário automático** (Agendador do Windows); recomendação (`recomendar.py`) já gera ações pros sinais que **não** dependem de margem (orçamento/ranking/ROAS vs. alvo); narrativa opcional (`narrar.py`, via `claude -p`) narra em português o que as duas camadas anteriores já calcularam, sem inventar conclusão nova. Recomendações condicionadas a margem seguem bloqueadas por não existir fonte de custo/margem por SKU. [[Product Ads — coletor (ads-monitor)]].
