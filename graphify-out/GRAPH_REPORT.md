...
## Corpus Check
- Corpus is ~35,288 words - fits in a single context window. You may not need a graph.

## Summary
- 757 nodes · 1330 edges · 45 communities (39 shown, 6 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 58 edges (avg confidence: 0.71)
...
- [[_COMMUNITY_Modelo Grupo + datas BR|Modelo Grupo + datas BR]]
...
- [[_COMMUNITY_Impressão em lote + estado|Impressão em lote + estado]]
- [[_COMMUNITY_Relatórios de texto|Relatórios de texto]]
- [[_COMMUNITY_Token Shopee (renovaçãoretry)|Token Shopee (renovação/retry)]]
- [[_COMMUNITY_Dias úteis  datas|Dias úteis / datas]]
...
- [[_COMMUNITY_Formato de etiqueta + Downloads|Formato de etiqueta + Downloads]]
...
- [[_COMMUNITY_OAuth Mercado Livre (setup)|OAuth Mercado Livre (setup)]]
- [[_COMMUNITY_Cache de envios + filtro|Cache de envios + filtro]]
- [[_COMMUNITY_Testes de config|Testes de config]]
- [[_COMMUNITY_Testes de resumo|Testes de resumo]]
...
1. `SeparadorApp` - 41 edges
2. `Grupo` - 28 edges
3. `main()` - 19 edges
4. `main()` - 16 edges
5. `Provedor` - 16 edges
6. `make_grupo()` - 16 edges
7. `cb_botao()` - 15 edges
8. `ProvedorMLAmbas` - 15 edges
9. `FakeResp` - 15 edges
10. `_grupo()` - 14 edges

## Surprising Connections (you probably didn't know these)
- `test_coletar_funde_e_soma_contagem()` --calls--> `Coleta`  [INFERRED]
  tests/test_ambas.py → separador_etiquetas_ml.py
- `grupos_de_detalhes()` --references--> `Grupo`  [EXTRACTED]
  shopee_api.py → separador_etiquetas_ml.py
- `imprimir_grupo()` --references--> `Grupo`  [EXTRACTED]
  shopee_api.py → separador_etiquetas_ml.py
- `marcar_impresso()` --references--> `Grupo`  [EXTRACTED]
  shopee_api.py → separador_etiquetas_ml.py
- `reimprimir_grupo()` --references--> `Grupo`  [EXTRACTED]
  shopee_api.py → separador_etiquetas_ml.py
...
### Community 0 - "Bot do Telegram"
Cohesion: 0.07
Nodes (65): _agendar_aviso(), _autorizado(), carregar_config(), cb_botao(), cmd_amanha(), cmd_conta(), cmd_desconhecido(), cmd_detalhar() (+57 more)

### Community 1 - "Interface gráfica (Tkinter)"
Cohesion: 0.06
Nodes (24): main(), separador_gui.py Telinha do Separador de Etiquetas do Mercado Livre. Mostra os g, Reconstroi o seletor de dia de despacho.          Sem `contagem` (antes do 1º At, Reconstroi os radios de conta. So aparece no ML com 2+ contas., Mostra/esconde a Identificação conforme o provedor (Shopee não carimba)., Troca o marketplace (ML/Shopee), recria o provedor e a tela inicial., Troca a conta ativa (ML) e volta para a tela inicial (sem buscar).         A bus, Mostra/esconde o rodape fixo do 'Imprimir selecionados' (fica sobre a         li (+16 more)

### Community 2 - "Camada de provedor (ML/Shopee/Ambas)"
Cohesion: 0.06
Nodes (15): criar_provedor(), fundir_grupos(), Provedor, ProvedorML, ProvedorMLAmbas, ProvedorShopee, provedores.py Abstrai o marketplace (Mercado Livre / Shopee) atras de uma interf, Funde grupos de varias contas por (SKU/chave + quantidade): um grupo unico     c (+7 more)
...
### Community 8 - "Testes de carimbo (DANFE)"
Cohesion: 0.13
...
### Community 9 - "Erros, credenciais e ZPL"
Cohesion: 0.12
Nodes (23): baixar_zpl(), carregar_credenciais(), envios_pendentes(), _gerar_zip(), gerar_zip_etiquetas(), gerar_zip_lotes(), imprimir_grupo(), imprimir_pendentes() (+15 more)

### Community 10 - "Testes do modo Ambas"
Cohesion: 0.15
...
### Community 11 - "Modelo Grupo + datas BR"
Cohesion: 0.15
Nodes (18): achar_grupo(), _amanha_br(), _chave_estado(), debug_envios(), detalhar(), Grupo, _hoje_br(), _impressos() (+10 more)

### Community 12 - "Persistência JSON (backup atômico)"
Cohesion: 0.16
Nodes (17): Path, _caminho_backup(), _carregar_credenciais_com_backup(), carregar_estado(), _gravar_credenciais_com_backup(), _gravar_json(), _ler_json(), _limpar_estado_antigo() (+9 more)

### Community 13 - "Shopee API: assinatura HMAC"
Cohesion: 0.18
Nodes (14): _assinar(), _assinatura_publica(), _assinatura_shop(), baixar_documento(), _download_shop(), envio_ja_arranjado(), _params_assinados(), shopee_api.py Integracao com a Shopee Open Platform API v2 — FASE 1 (somente lei (+6 more)

### Community 14 - "Testes de lotes/carimbo"
Cohesion: 0.26
...
### Community 15 - "Carimbo por nome/SKU"
Cohesion: 0.14
Nodes (14): _carimbar_grupo(), carimbar_zpl(), carregar_nomes(), _fonte_nome(), Carimba `texto` (o SKU ou o nome do produto) na DANFE (nota fiscal),     CENTRAL, Etiqueta separadora (1 pagina ZPL) com SKU, nome e quantidade do lote,     centr, Le o de-para SKU -> nome do nomes_sku.json (vazio se nao existir)., Codigo curto do produto para carimbar na etiqueta.      Grupo normal: o proprio (+6 more)

### Community 16 - "Shopee: ship_order e rastreio"
Cohesion: 0.15
...
### Community 17 - "Busca de pedidos (ML)"
Cohesion: 0.21
...
### Community 18 - "Impressão em lote + estado"
Cohesion: 0.19
Nodes (13): _aguardar_awbs(), imprimir_grupo(), imprimir_lotes(), marcar_impresso(), _organizar_varios(), _rastreios_paralelo(), Busca o AWB de varios pedidos EM PARALELO. Devolve {order_sn: awb} ('' em     fa, Espera os AWBs de varios pedidos sairem (a Shopee leva alguns segundos     apos (+5 more)
...
### Community 20 - "Token Shopee (renovação/retry)"
Cohesion: 0.20
...
### Community 21 - "Dias úteis / datas"
Cohesion: 0.22
Nodes (7): date, proximos_dias_uteis(), As proximas `qtd` datas UTEIS (segunda a sexta) a partir de hoje     (inclusive), Datas no horario de Brasilia (filtro de despacho)., test_proximos_dias_uteis_comeca_no_proximo_util_no_sabado(), test_proximos_dias_uteis_dia_comum(), test_proximos_dias_uteis_numa_sexta_pula_o_fim_de_semana()

### Community 22 - "Editor de nomes (GUI)"
Cohesion: 0.29
...
### Community 23 - "Shopee: documento em lote (batch)"
Cohesion: 0.20
Nodes (11): batch_ship_order(), criar_documento(), _gerar_bloco(), _post_shop(), POST assinado em uma API de loja (sign na query, dados no corpo JSON).     Passa, Organiza VARIOS envios num request so (ate LOTE_SHIP), como Postagem     (drop-o, Cria o documento da etiqueta. A Shopee exige o tracking_number (AWB) de cada, Extrai {order_sn: status} do retorno do get_shipping_document_result. (+3 more)

### Community 24 - "Testes de multi-conta"
Cohesion: 0.33
...
### Community 25 - "Testes do pipeline de coleta"
Cohesion: 0.36
...
### Community 26 - "Config e multi-conta (núcleo)"
Cohesion: 0.20
...
### Community 27 - "Shopee: detalhes/agrupamento"
Cohesion: 0.28
...
Cohesion: 0.22
Nodes (9): carregar_credenciais(), main(), obter_token(), parametros_documento(), preencher_rastreios(), Token valido do cache, ou renova. Serializa o refresh com um lock e     re-checa, Tipos de documento disponiveis para o pedido (para conferir o que da pra gerar)., Para grupos de UM unico pedido JA IMPRESSO, busca o AWB (get_tracking_number) (+1 more)

### Community 29 - "Shopee: geração de etiqueta"
Cohesion: 0.29
Nodes (8): _combinar_etiquetas(), gerar_etiqueta(), _gerar_lote(), Gera (assincrono) e baixa as etiquetas dos pedidos. So baixa quando TODOS     os, Extrai o ZPL (em BYTES, sem reencodar — evita corromper o ~DG/Z64) de dentro, Junta o ZPL de varias etiquetas Shopee num UNICO .zip (um TXT) — para a     Zebr, Gera as etiquetas dos pedidos `alvo` num so ZIP, tolerando falha parcial.     Te, _zpl_do_zip()
...
### Community 32 - "Formato de etiqueta + Downloads"
Cohesion: 0.29
Nodes (7): detectar_formato(), Identifica o formato do arquivo baixado pelos primeiros bytes.      A etiqueta t, Grava a etiqueta na pasta Downloads e devolve (caminho, formato detectado)., Regera a etiqueta de TODOS os envios do grupo, sem mexer no estado (util     qua, reimprimir_grupo(), _rotulo_lote(), salvar_etiqueta()

### Community 34 - "Testes de paginação de busca"
Cohesion: 0.43
...
### Community 35 - "Testes de cache de envios"
Cohesion: 0.52
...
### Community 37 - "OAuth Mercado Livre (setup)"
Cohesion: 0.47
...
### Community 38 - "Cache de envios + filtro"
Cohesion: 0.33
Nodes (6): _carregar_envios_cache(), filtrar_para_imprimir(), _limpar_envios_cache(), Descarta entradas mais antigas que a janela de busca de pedidos     (alem dela o, Mantem pedidos em ready_to_print. progresso(feitos, total) e chamado por pedido., _salvar_envios_cache()

### Community 40 - "Testes de resumo"
Cohesion: 0.40
...
## Knowledge Gaps
- **3 isolated node(s):** `session-start.sh script`, `separador-etiquetas-ml`, `setup_gui_tests.sh script`
...
- **Why does `SeparadorApp` connect `Interface gráfica (Tkinter)` to `Editor de nomes (GUI)`?**
...
- **Should `Camada de provedor (ML/Shopee/Ambas)` be split into smaller, more focused modules?**
  _Cohesion score 0.06038961038961039 - nodes in this community are weakly interconnected._
- **Should `Testes da Shopee` be split into smaller, more focused modules?**
